...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = df[df.education.isin(education)]
    lower_education = df[~df.education.isin(education)]
    ...
    all_salaries = (
        df[df["native-country"] != "?"].groupby("native-country")["salary"].count()
    )
    salary_over50k_perc = over50k / all_salaries * 100
    highest_earning_country = salary_over50k_perc.sort_values(ascending=False).index[0]
    highest_earning_country_percentage = salary_over50k_perc.sort_values(
        ascending=False
    )[0].round(1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = (
        df[["occupation", "native-country", "salary"]][
